...
public class ManagementPrison {
	...
	private void selectLocation() {
		...
		List<Management> info = new ArrayList<Management>();
		// 수용중인 인원(가석방이나 
		for (Prison imprisonList : info) {
			// System.out.println(prison);
			int people = pDAO.selectInfo(prison.getPrisonLocation());
			int freedomNum = pDAO.selectFreedom(prison.getPrisonLocation());
			prison.setPrisonOccupy(prison.getPrisonAccommodate() - people + freedomNum);
		...
		List<Management> offender = gDAO.distinctPrisonLocation(prisonLocation);
		try {
			if (offender == null) {
				System.out.println("해당지역에 수감된 범죄자가 없습니다.");
			}
			System.out.println("해당지역에 수감된 범죄자입니다.");

			for (Management offenderList : offender) {
				System.out.println(offenderList.toString());
			}
		} catch (NullPointerException e) {
			System.out.println("해당지역에 수감자 없음");
		}
